            ...
            <tbody className="divide-y divide-gray-50">
              {tradeIns.map((item, i) => (
                <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-2 py-2"><input data-col="resale_price" data-row={i} type="text" inputMode="numeric" disabled={item.resale_locked} title={item.resale_locked ? t.quotation_resale_locked : undefined} value={formatNumber(item.resale_price)} onChange={(e) => updateTradeIn(i, "resale_price", parseNumber(e.target.value))} onKeyDown={(e) => handleTabDown(e, "resale_price", i)} className={`border border-gray-200 rounded-lg px-2.5 py-1.5 w-full text-right text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 ${item.resale_locked ? "bg-gray-100 text-gray-400 cursor-not-allowed" : ""}`} /></td>
                  <td className="px-1 py-2 text-center">{!item.resale_locked && (<button type="button" onClick={() => setTradeIns(tradeIns.filter((_, j) => j !== i))} className="p-1 rounded-md hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>)}</td>
                </tr>
              ))}
            </tbody>
          </table>
